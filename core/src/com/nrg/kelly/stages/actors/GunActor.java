...
public class GunActor extends GameActor{

    private Vector2 linearVelocity;
    private boolean pickedUp = false;

    public GunActor(GunConfig config, CameraConfig cameraConfig) {
        super(config, cameraConfig);
        Events.get().register(this);
        final Body body = Box2dFactory.getInstance().createGun(config);
